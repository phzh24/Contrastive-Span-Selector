"""
from: [haonan-li/MultiSpanQA: MultiSpanQA: A Dataset for Multi-Span Question Answering](https://github.com/haonan-li/MultiSpanQA)
"""
...
def count_overlap(gold: set, pred: set):
    ...
    for i, gold_str in enumerate(gold):
        for j, pred_str in enumerate(pred):
            ...
            _, _, longest_len = seq_matcher.find_longest_match(
                0, len(gold_str), 0, len(pred_str)
            )
...
def multi_span_evaluate(preds: Dict[str, Set], golds: Dict[str, Set]):
    ...
    result = {
        "exact_match_precision": 100 * em_p,
...
def my_evaluate_one_sample(context, label, pred):
    golden = get_entities(label, context)
    pred_span = get_entities(pred, context)
    # print(golden, pred_span)
    p1, r1, f1 = compute_scores(
        {"one": set(golden)}, {"one": set(pred_span)}, eval_type="em"
    )
    p2, r2, f2 = compute_scores(
        {"one": set(golden)}, {"one": set(pred_span)}, eval_type="pm"
    )
    return (p1, r1, f1), (p2, r2, f2)


def my_evaluate_one_sample_to_set(context, label, pred):
    golden = get_entities(label, context)
    pred_span = get_entities(pred, context)
    golden = set([g[0] for g in golden])
    pred_span = set([p[0] for p in pred_span])
    # print(golden, pred_span)
    p1, r1, f1 = compute_scores({"one": golden}, {"one": pred_span}, eval_type="em")
    p2, r2, f2 = compute_scores({"one": golden}, {"one": pred_span}, eval_type="pm")
    return (p1, r1, f1), (p2, r2, f2)
...
def apply_threshold_nbest(threshold, nbest_preds):
    ...
    for k, v in nbest_preds.items():
        other_pred = filter(
            lambda x: x["probability"] >= threshold, nbest_preds[k][1:]
        )  # other preds except the first one
        if nbest_preds[k][0]["text"] != "":  # only apply to the has_answer examples
            preds[k] = list(
                set(
                    [nbest_preds[k][0]["text"]]
                    + list(map(lambda x: x["text"], other_pred))
                )
            )
        else:
            preds[k] = [""]
